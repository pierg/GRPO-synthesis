...
def deserialize_value(value: Any, target_type: Type) -> Any:
    ...
    if target_type in (str, int, float, bool, type(None)):
        ...
        return target_type(deserialize_value(item, target_type.__args__[0]) for item in value)
    elif target_type == dict:
        return {k: deserialize_value(v, target_type.__args__[1]) for k, v in value.items()}
